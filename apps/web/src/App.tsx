...
function Metadata(){
  ...
  useEffect(()=>{
    ...
    const names:Record<string,string>={'/':'The trusted mechanic network','/mechanics':'Find a mechanic','/for-mechanics':'Built for automotive professionals','/account':'Account','/vehicles':'Vehicles','/auth':'Sign in'};
    ...
    let robots=document.querySelector<HTMLMetaElement>('meta[name="robots"]');if(!robots){robots=document.createElement('meta');robots.name='robots';document.head.append(robots);}robots.content=['/auth','/account','/vehicles'].includes(path)||location.search?'noindex, follow':'index, follow';
