...
for(const route of ['for-mechanics','auth','account','vehicles']){mkdirSync(resolve(dist,route),{recursive:true});writeFileSync(resolve(dist,route,'index.html'),index.replace('<link rel="canonical" href="https://www.cnmechanic.com/"/>',`<link rel="canonical" href="${canonicalOrigin}/${route}"/>`));}
writeFileSync(resolve(dist,'404.html'),index);
writeFileSync(resolve(dist,'robots.txt'),'User-agent: *\nAllow: /\nSitemap: https://www.cnmechanic.com/sitemap.xml\n');
writeFileSync(resolve(dist,'sitemap.xml'),'<?xml version="1.0" encoding="UTF-8"?>\n<urlset xmlns="http://www.sitemaps.org/schemas/sitemap/0.9"><url><loc>https://www.cnmechanic.com/</loc></url><url><loc>https://www.cnmechanic.com/for-mechanics</loc></url></urlset>\n');
writeFileSync(resolve(dist,'llms.txt'),'# CNMechanic\n\nCNMechanic is a trusted mechanic network. Customer accounts and private vehicle access are live. Mechanic search and booking are coming in a future release.\n\n- Professional network: /for-mechanics\n- OpenAPI: https://api.cnmechanic.com/openapi.json\n- WebMCP: get_platform_info, public read only, when supported.\n- Remote MCP: planned, not available.\n');
