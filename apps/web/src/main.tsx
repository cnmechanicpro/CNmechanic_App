...
import './styles.css';
async function start(){const [{default:App},{AuthProvider},{api},{marketplaceRegistry,registerBrowserTools}]=await Promise.all([import('./App'),import('./auth'),import('./services'),import('@cnmechanic/webmcp')]);createRoot(document.getElementById('root')!).render(<StrictMode><ErrorBoundary><BrowserRouter><AuthProvider><App/></AuthProvider></BrowserRouter></ErrorBoundary></StrictMode>);const controller=new AbortController();window.addEventListener('pagehide',()=>controller.abort(),{once:true});void registerBrowserTools(marketplaceRegistry(api),document,controller.signal);}
