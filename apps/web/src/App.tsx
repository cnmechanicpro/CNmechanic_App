...
const makes=['BMW','Mercedes-Benz','Audi','Volkswagen','Volvo','Porsche','MINI','Land Rover','Jaguar'];
...
function Metadata(){
  ...
  useEffect(()=>{
    ...
    const names:Record<string,string>={'/':'The trusted mechanic network','/for-mechanics':'Built for automotive professionals','/account':'Account','/vehicles':'Vehicles','/auth':'Sign in'};
    const descriptions:Record<string,string>={'/':'Find trusted automotive specialists, organize your vehicle history, and move through every service with clarity.','/for-mechanics':'Join the trusted network built to help automotive professionals get discovered, build trust, and keep customers coming back.'};
    const canonical=document.querySelector<HTMLLinkElement>('link[rel="canonical"]');
    const description=document.querySelector<HTMLMetaElement>('meta[name="description"]');
    if(canonical)canonical.href=canonicalUrl(path);
    if(description&&descriptions[path])description.content=descriptions[path];
    document.title=`${names[path]??'Page not found'} | CNMechanic`;
...
function SearchModule(){
  ...
  function submit(e:FormEvent<HTMLFormElement>){e.preventDefault();void navigate('/auth');}
  return <form className="search-module" id="find" onSubmit={submit}>
    <div className="search-heading"><div><span className="section-kicker">EARLY ACCESS</span><h2>Find the right specialist.</h2></div><p>Search and booking are coming next. Create your account to be ready.</p></div>
    <div className="search-fields">
      <label><span>Location</span><input name="location" placeholder="City, state, or ZIP" autoComplete="postal-code"/></label>
      <label><span>Vehicle</span><select name="vehicle" defaultValue=""><option value="" disabled>Make or model</option>{makes.map(make=><option key={make}>{make}</option>)}</select></label>
      <label><span>Service</span><select name="service" defaultValue=""><option value="" disabled>What do you need?</option>{services.map(([,name])=><option key={name}>{name}</option>)}</select></label>
      <Button type="submit">Join early access <Icon name="arrow" size={18}/></Button>
    </div>
  </form>;
}
...
function SiteFooter(){return <footer className="site-footer"><div className="footer-inner"><div><Brand footer/><p>A higher standard for automotive care.</p></div><div><strong>Explore</strong><a href="/#find">Find a mechanic</a><a href="/#how-it-works">How it works</a><Link to="/for-mechanics">For mechanics</Link></div><div><strong>Your account</strong><Link to="/auth">Sign in</Link><Link to="/vehicles">CN Garage</Link><Link to="/account">Account settings</Link></div><div><strong>Foundation status</strong><span>Accounts and private vehicle access are live.</span><span>Search and booking are coming next.</span></div></div><div className="footer-bottom"><span>© {new Date().getFullYear()} CNMechanic. All rights reserved.</span><span>Better cars. Brighter journeys.</span></div></footer>}

export default function App(){const [menu,setMenu]=useState(false);const {session}=useAuth();const closeMenu=()=>setMenu(false);return <><Metadata/><a className="skip-link" href="#main">Skip to content</a><header className="site-header"><div className="nav"><Brand/><button className="menu-button" aria-expanded={menu} aria-controls="primary-nav" onClick={()=>setMenu(!menu)}>Menu</button><nav id="primary-nav" aria-label="Primary" className={menu?'open':''}><a href="/#find" onClick={closeMenu}>Find a mechanic</a><a href="/#how-it-works" onClick={closeMenu}>How it works</a><NavLink to="/for-mechanics" onClick={closeMenu}>For mechanics</NavLink></nav><div className="nav-actions"><NavLink className="nav-signin" to={session?'/account':'/auth'}>{session?'Account':'Sign in'}</NavLink><Link className="button nav-cta" to={session?'/vehicles':'/auth'}>{session?'My garage':'Create account'}</Link></div></div></header><main id="main" tabIndex={-1}><Routes><Route path="/" element={<Home/>}/><Route path="/for-mechanics" element={<ForMechanics/>}/><Route path="/auth" element={<AuthPage/>}/><Route path="/account" element={<Account/>}/><Route path="/vehicles" element={<Vehicles/>}/><Route path="*" element={<div className="narrow error-page"><p className="eyebrow line-label">404 / LOST ROAD</p><h1>Page not found.</h1><p>The page you’re looking for does not exist.</p><Link className="button" to="/">Return home <Icon name="arrow" size={18}/></Link></div>}/></Routes></main><SiteFooter/></>}
