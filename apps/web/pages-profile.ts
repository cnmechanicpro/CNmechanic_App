...
export async function renderPublicProfile(context:PagesContext,kind:'mechanics'|'shops'){
 ...
 let html=await shell.text();html=html.replace('<title>CNMechanic | The Trusted Mechanic Network</title>',`<title>${escape(name)} | CNMechanic</title>`).replace('<link rel="canonical" href="https://www.cnmechanic.com/"/>',`<link rel="canonical" href="${canonical}"/>`).replace(/<meta name="description" content="[^"]*"\/>/,`<meta name="description" content="${escape(description)}"/>`);
 ...
 html=html.replace('</head>',`<script type="application/ld+json">${json}</script></head>`);return new Response(html,{status:200,headers:{...Object.fromEntries(shell.headers),'Content-Type':'text/html; charset=utf-8','Cache-Control':'public, max-age=300'}});
}
