...
export default {async fetch(request:Request,env:Env):Promise<Response>{
 ...
 try{
  ...
  if(request.method==='OPTIONS'){
   ...
   if(!origin || !method || !['GET','PATCH'].includes(method) || headers.some(h=>!['authorization','content-type'].includes(h)))throw new AppError('FORBIDDEN',403,'Preflight is not permitted.');
   response=new Response(null,{status:204,headers:{'Access-Control-Allow-Methods':'GET, PATCH','Access-Control-Allow-Headers':'Authorization, Content-Type','Access-Control-Max-Age':'600'}});
